...
def createCraftFiles():
    ...
    for file in item_file_list:
        item_ids.append((file.strip(' '))[:-5])

    print(item_ids)

    try:
        os.mkdir('./create_craft_files')
    except FileExistsError:
        print('Craft files directory already exists. Proceeding...')
        pass

    for item in item_ids:
        print(item)
        with open(f'unique_crafts/{item}.json', 'w') as file:
