execute unless score #I0127 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.I0127] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.I0127
tag @e[tag=lk.goaltracker] add lk.I0127
scoreboard players set @s lk.goal 730127
...
say completed "Obtain Iron Bars"
