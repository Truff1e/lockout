execute unless score #N0004 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.N0004] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.N0004
tag @e[tag=lk.goaltracker] add lk.N0004
scoreboard players set @s lk.goal 780004
...
say failed "Opponent Catches Fire"
