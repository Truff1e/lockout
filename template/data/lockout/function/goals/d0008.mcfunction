execute unless score #D0008 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.D0008] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.D0008
tag @e[tag=lk.goaltracker] add lk.D0008
scoreboard players set @s lk.goal 680008
...
say completed "Die to TNT Minecart"
