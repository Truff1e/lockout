...
execute unless score #A0011 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.A0011] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.A0011
tag @e[tag=lk.goaltracker] add lk.A0011
scoreboard players set @s lk.goal 650011
...
say completed "Trade with a Villager"
