execute unless score #I0045 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.I0045] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.I0045
tag @e[tag=lk.goaltracker] add lk.I0045
scoreboard players set @s lk.goal 730045
...
say completed "Obtain Recovery Compass"
