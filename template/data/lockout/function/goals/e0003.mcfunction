execute unless score #E0003 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.E0003] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.E0003
tag @e[tag=lk.goaltracker] add lk.E0003
scoreboard players set @s lk.goal 690003
