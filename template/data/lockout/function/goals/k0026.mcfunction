execute unless score #K0026 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.K0026] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.K0026
tag @e[tag=lk.goaltracker] add lk.K0026
scoreboard players set @s lk.goal 750026
...
say completed "Kill Wither Skeleton"
