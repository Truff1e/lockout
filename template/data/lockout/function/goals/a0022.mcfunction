execute unless score #A0022 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.A0022] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.A0022
tag @e[tag=lk.goaltracker] add lk.A0022
scoreboard players set @s lk.goal 650022
...
say completed "Get 35 Unique Advancements"
