execute unless score #B0008 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.B0008] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.B0008
tag @e[tag=lk.goaltracker] add lk.B0008
scoreboard players set @s lk.goal 660008
