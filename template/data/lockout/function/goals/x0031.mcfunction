execute unless score #X0031 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.X0031] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.X0031
tag @e[tag=lk.goaltracker] add lk.X0031
scoreboard players set @s lk.goal 880031
...
say completed "Get Water Breathing"
