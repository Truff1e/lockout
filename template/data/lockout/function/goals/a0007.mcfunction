...
execute unless score #A0007 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.A0007] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.A0007
tag @e[tag=lk.goaltracker] add lk.A0007
scoreboard players set @s lk.goal 650007
...
say completed "Get Sniper Duel Advancement"
