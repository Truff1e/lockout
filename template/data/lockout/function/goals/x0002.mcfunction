execute unless score #X0002 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.X0002] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.X0002
tag @e[tag=lk.goaltracker] add lk.X0002
scoreboard players set @s lk.goal 880002
...
say completed "Get 64 of the Same Item"
