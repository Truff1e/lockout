execute unless score #K0007 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.K0007] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.K0007
tag @e[tag=lk.goaltracker] add lk.K0007
scoreboard players set @s lk.goal 750007
...
say completed "Kill Elder Guardian"
