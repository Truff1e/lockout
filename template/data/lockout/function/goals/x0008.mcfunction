execute unless score #X0008 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.X0008] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.X0008
tag @e[tag=lk.goaltracker] add lk.X0008
scoreboard players set @s lk.goal 880008
...
say completed "Mine Mob Spawner"
