...
execute unless score #A0005 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.A0005] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.A0005
tag @e[tag=lk.goaltracker] add lk.A0005
scoreboard players set @s lk.goal 650005
...
say completed "Get Oh Shiny Advancement"
