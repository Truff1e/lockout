execute unless score #I0105 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.I0105] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.I0105
tag @e[tag=lk.goaltracker] add lk.I0105
scoreboard players set @s lk.goal 730105
...
say completed "Obtain Netherite Ingot"
