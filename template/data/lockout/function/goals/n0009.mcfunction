execute unless score #N0009 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.N0009] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.N0009
tag @e[tag=lk.goaltracker] add lk.N0009
scoreboard players set @s lk.goal 780009
...
say failed "Opponent Wears Armor"
