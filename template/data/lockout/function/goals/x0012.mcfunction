execute unless score #X0012 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.X0012] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.X0012
tag @e[tag=lk.goaltracker] add lk.X0012
scoreboard players set @s lk.goal 880012
...
say completed "Mine Emerald Ore"
