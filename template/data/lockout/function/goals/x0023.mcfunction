execute unless score #X0023 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.X0023] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.X0023
tag @e[tag=lk.goaltracker] add lk.X0023
scoreboard players set @s lk.goal 880023
...
say completed "Have 9 Status Effects at Once"
