execute unless score #K0027 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.K0027] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.K0027
tag @e[tag=lk.goaltracker] add lk.K0027
scoreboard players set @s lk.goal 750027
...
say completed "Kill 7 Unique Hostile Mobs"
