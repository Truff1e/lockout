execute unless score #K0028 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.K0028] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.K0028
tag @e[tag=lk.goaltracker] add lk.K0028
scoreboard players set @s lk.goal 750028
...
say completed "Kill 15 Unique Hostile Mobs"
