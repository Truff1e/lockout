execute unless score #L0006 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.L0006] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.L0006
tag @e[tag=lk.goaltracker] add lk.L0006
scoreboard players set @s lk.goal 760006
