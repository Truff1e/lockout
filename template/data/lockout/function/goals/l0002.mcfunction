execute unless score #L0002 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.L0002] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.L0002
tag @e[tag=lk.goaltracker] add lk.L0002
scoreboard players set @s lk.goal 760002
