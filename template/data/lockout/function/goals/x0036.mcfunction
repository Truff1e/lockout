execute unless score #X0036 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.X0036] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.X0036
tag @e[tag=lk.goaltracker] add lk.X0036
scoreboard players set @s lk.goal 880036
...
say completed "Craft 25 Unique Items"
