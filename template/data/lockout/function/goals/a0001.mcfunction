execute unless score #A0001 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.A0001] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.A0001
tag @e[tag=lk.goaltracker] add lk.A0001
scoreboard players set @s lk.goal 650001
...
say completed "Get 25 Unique Advancements"
