execute unless score #N0006 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.N0006] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.N0006
tag @e[tag=lk.goaltracker] add lk.N0006
scoreboard players set @s lk.goal 780006
...
say failed "Opponent Obtains Obsidian"
