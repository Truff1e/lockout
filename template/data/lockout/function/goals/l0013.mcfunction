execute unless score #L0013 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.L0013] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.L0013
tag @e[tag=lk.goaltracker] add lk.L0013
scoreboard players set @s lk.goal 760013
...
say completed "Locate Woodland Mansion"
