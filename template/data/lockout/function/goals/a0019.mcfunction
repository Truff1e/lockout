...
execute unless score #A0019 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.A0019] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.A0019
tag @e[tag=lk.goaltracker] add lk.A0019
scoreboard players set @s lk.goal 650019
...
say completed "Get "Caves and Cliffs" Advancement"
