execute unless score #K0010 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.K0010] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.K0010
tag @e[tag=lk.goaltracker] add lk.K0010
scoreboard players set @s lk.goal 750010
...
say completed "Kill Green Sheep"
