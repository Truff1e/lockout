execute unless score #N0003 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.N0003] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.N0003
tag @e[tag=lk.goaltracker] add lk.N0003
scoreboard players set @s lk.goal 780003
...
say failed "Opponent Takes Fall Damage"
