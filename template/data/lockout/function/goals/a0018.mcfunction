...
execute unless score #A0018 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.A0018] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.A0018
tag @e[tag=lk.goaltracker] add lk.A0018
scoreboard players set @s lk.goal 650018
...
say completed "Get "Power of Books" Advancement"
