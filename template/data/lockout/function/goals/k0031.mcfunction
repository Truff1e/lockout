execute unless score #K0031 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.K0031] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.K0031
tag @e[tag=lk.goaltracker] add lk.K0031
scoreboard players set @s lk.goal 750031
...
say completed "Kill Zombie Villager"
