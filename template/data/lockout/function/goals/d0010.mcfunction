execute unless score #D0010 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.D0010] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.D0010
tag @e[tag=lk.goaltracker] add lk.D0010
scoreboard players set @s lk.goal 680010
...
say completed "Die to Polar Bear"
