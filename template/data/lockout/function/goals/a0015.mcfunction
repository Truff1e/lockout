...
execute unless score #A0015 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.A0015] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.A0015
tag @e[tag=lk.goaltracker] add lk.A0015
scoreboard players set @s lk.goal 650015
...
say completed "Get Subspace Bubble Advancement"
