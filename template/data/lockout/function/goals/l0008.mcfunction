execute unless score #L0008 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.L0008] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.L0008
tag @e[tag=lk.goaltracker] add lk.L0008
scoreboard players set @s lk.goal 760008
...
say completed "Locate all Nether Biomes"
