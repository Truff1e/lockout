execute unless score #K0013 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.K0013] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.K0013
tag @e[tag=lk.goaltracker] add lk.K0013
scoreboard players set @s lk.goal 750013
...
say completed "Kill Light Blue Sheep"
