execute unless score #E0010 lk.enabled_goals matches 1 run return fail
execute if score #end_seen lk.util matches 1 run return fail
execute if entity @e[tag=lk.E0010] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.E0010
tag @e[tag=lk.goaltracker] add lk.E0010
scoreboard players set @s lk.goal 690010
...
say completed "Eat Suspicious Stew"
