...
execute unless score #A0017 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.A0017] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.A0017
tag @e[tag=lk.goaltracker] add lk.A0017
scoreboard players set @s lk.goal 650017
...
say completed "Fully Charge Respawn Anchor"
