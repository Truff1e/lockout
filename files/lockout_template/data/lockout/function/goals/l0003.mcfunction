execute unless score #L0003 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.L0003] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.L0003
tag @e[tag=lk.goaltracker] add lk.L0003
scoreboard players set @s lk.goal 760003
...
say completed "Reach Height Limit"
