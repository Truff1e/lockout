...
execute unless score #A0009 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.A0009] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.A0009
tag @e[tag=lk.goaltracker] add lk.A0009
scoreboard players set @s lk.goal 650009
...
say completed "Do Star Trader Advancement"
