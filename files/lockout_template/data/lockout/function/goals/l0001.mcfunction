execute unless score #L0001 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.L0001] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.L0001
tag @e[tag=lk.goaltracker] add lk.L0001
scoreboard players set @s lk.goal 760001
...
say completed "Find an Ancient City"
