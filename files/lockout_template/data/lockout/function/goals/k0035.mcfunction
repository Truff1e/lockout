execute unless score #K0035 lk.enabled_goals matches 1 run return fail
execute if entity @e[tag=lk.K0035] run return fail
execute if entity @s[team=spectator] run return fail
tag @s add lk.K0035
tag @e[tag=lk.goaltracker] add lk.K0035
scoreboard players set @s lk.goal 750035
...
say completed "Kill Zombified Piglin"
